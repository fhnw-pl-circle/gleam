...
function pad_list(list, size) {
  ...
  return append3(
    _pipe,
    repeat2(new None(), extra_required(list, size))
  );
}
...
function drop_last(elements2) {
  return take(elements2, length3(elements2) - 1);
}
        ...
        if ($1 === "") {
          ...
          let path_segments$1 = (() => {
            ...
            if ($2) {
              ...
              return append3(_pipe$1, split3(relative.path, "/"));
            }
          })();
          let path = (() => {
...
// build/dev/javascript/echo_client/echo_client.mjs
var Model2 = class extends CustomType {
  constructor(text2, messages2, connection, socket) {
    super();
    this.text = text2;
...
var SetText = class extends CustomType {
  constructor(text2) {
    super();
    this.text = text2;
  }
...
function update(model, msg) {
  ...
  if (msg instanceof SetText) {
    let text2 = msg.text;
    return [model.withFields({ text: text2 }), none()];
  } else if (msg instanceof Connect) {
    return [
      model.withFields({ connection: new Connecting() }),
      init2(
        "http://localhost:3000/ws",
        (var0) => {
          return new WsWrapper(var0);
        }
      )
    ];
  } else if (msg instanceof Send && $ instanceof Some) {
    let socket = $[0];
    return [model.withFields({ text: "" }), send(socket, model.text)];
  } else if (msg instanceof WsWrapper && msg[0] instanceof OnTextMessage) {
    let text2 = msg[0][0];
    return [
      model.withFields({ messages: append3(model.messages, toList([text2])) }),
      none()
    ];
  } else if (msg instanceof WsWrapper && msg[0] instanceof OnOpen) {
    let socket = msg[0][0];
    return [
      model.withFields({
        connection: new Connected(),
        socket: new Some(socket)
      }),
      none()
    ];
  } else {
    throw makeError("panic", "echo_client", 105, "update", "\u{1F4A5}", {});
  }
}
function button2(model) {
  ...
  if ($ instanceof Disconnected) {
    ...
    return button(
      ...
      toList([text("connecting...")])
    );
  } else {
    return button(
      toList([on_click(new Send())]),
      toList([text("send")])
    );
  }
}
function messages(model) {
  let list = (() => {
    ...
    return map2(
      _pipe,
      (text2) => {
        return ul(toList([]), toList([text(text2)]));
      }
    );
  })();
  return ul(toList([]), list);
}
function view(model) {
  return div(
    toList([]),
